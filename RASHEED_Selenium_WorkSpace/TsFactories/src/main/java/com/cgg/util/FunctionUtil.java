...
public static void textBox(WebElement element,String enterText)
{
...
public static void clickOnButton(WebElement element)
{
	...
	try {
		...
		System.out.println("Ele ment is not attached to the page document "
				+ e.getStackTrace());
	} catch (NoSuchElementException e) {
		System.out.println("Element " + element + " was not found in DOM "
				+ e.getStackTrace());
		...
		//APP_LOGS.info("Accept UnExpected Alert If Available");
				...
				do
				{
					try
					{
						...
						if (alert != null)
						{
							 alerttext=alert.getText().trim().substring(0,35);
							APP_LOGS.info(" Alert Is Present:-"+alerttext);
//							//createScreenshotUsingRobot(alerttext.trim()+name);
//							alert.accept();
							boolFound = true;
