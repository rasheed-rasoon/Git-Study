...
public class FunctionUtil extends TestBase{
	...
	public static void moveMouseToElement(WebElement element) {
		APP_LOGS.info("Mouse Hover(Move)  on the element "+ element.getText());
		...
		do
		{
			try
			{
				...
				if (alert != null)
				{
					...
					APP_LOGS.info(" Alert Is Present:-"+getAlerttext);
					//createScreenshotUsingRobot(alerttext.trim()+name);
					alert.accept();
					test.log(LogStatus.INFO,"alert preent and alert text is "+alerttext);
					boolFound = true;
				}
			}
