...
public class GHMCTeamLoginTest extends TestBase{
	...
	public void testLogin(Hashtable <String,String>data){
		...
		driver.get("testghmc.cgg.gov.in/wms");
		...
		if(data.get("Dv.inataFlag").equalsIgnoreCase("P") && home.isHomePageOpened()){
			reportFail("unable to logged for valid data");
		}
		else 
			
			if(data.get("DataFlag").equalsIgnoreCase("N") && lp.isEntreprenuerLoggedInPageOpened()){
